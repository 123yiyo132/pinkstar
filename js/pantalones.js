...
// ================== CARGAR JSON ==================
fetch("json/dataPantalones.json")
  .then(res => res.json())
  .then(data => {
    productos = data;
  ...
  .catch(err => console.error("Error cargando pantalones:", err));
...
function verProducto(titulo) {
  ...
  productoSeleccionado.tallas.forEach(talla => {
    ...
    btn.className = "talla-option btn btn-outline-light";
    btn.innerText = talla;
...
document.querySelectorAll(".filtro").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    if (filtro === "Todos") {
      ...
      productosFiltrados = productos.filter(p => p.estilo === filtro);
    }
